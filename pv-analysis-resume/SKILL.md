...
| `figures/**/*.stats.json` + `ANALYSIS.md` | 对应图的重画 | 先读旧 `stats.json` 与旧结论，够用就引用（不 Read PNG） |
...
#1/#2/#4/#8）、每图读 stats.json 的完整曲线/矩阵/分位数（**不 Read PNG**）→ 对号
`references/figure-diagnostics.md` 形态 → 结论、沉淀到 `figures/<电站>/<范围>/ANALYSIS.md`
与 `FINDINGS.md`。到 Stage 3 结束停下报现象清单。
...
3. 连 ANALYSIS.md 都没有，只有 `.stats.json`（PNG 只是给人看的副产物）→ 逐张读 stats.json，
   由本会话现读现总结出现象清单，同样回填 FINDINGS.md。
4. 连 stats.json 都没有 → 无既有事实可归因，**退回路径 A 先跑 Stage 2–3**。

**进入前提（无论现象来自哪一级都要满足）**：现象引用的图的 `.stats.json` 仍在
`figures/` 下可读——Stage 4 要复核这些证据数字与形态（曲线/矩阵/分位数都在 json 里，
不 Read PNG），不是另起炉灶凭空编故事。

**直达 Stage 4 读什么、做什么**（Playbook 全文见 `references/playbooks.md`）：
1. 按上面优先级拿到现象清单；**本次归因直接依据的那几张图，读它们的 stats.json**
   （完整曲线 + 形状描述符 + 分位数已承载散点弯曲、坏天聚集等形状证据，读 json 即可判形态，
   **不 Read PNG**——读图费上下文），**这些图已在，通常无需重画**。
...
### 通用
- 产物写回**同一** figures/ 目录结构；将要覆盖同名旧图时，先读旧 `stats.json` 和旧结论，
  确认是刷新而不是误覆盖不同范围的分析。
