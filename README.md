...
单一入口技能 —— 一个泛化时序诊断引擎，9 个可插拔 playbook：

| 技能 | 用途 |
|------|------|
| [`ts-diagnose`](ts-diagnose/SKILL.md) | **泛化时序诊断引擎**：同一套已验证 workflow（orient 续跑、事实/解释隔离、结论三道门、多证据线、提问一等公民、生成闸+归因闸）+ 9 个可插拔 playbook，覆盖时序/预测任务的全部诊断与评估目标；成熟运行可经 crystallize 固化成薄的已固化代理技能 |
...
| `model-comparison` | 为什么模型 A 比 B 好/差、多模型对比归因 |
| `deployment-drift` | 上线/部署后是不是退化了、误差从什么时候开始变大、漂移诊断 |
| `fact-scan` | 只想体检/把标准分析图画一遍/看现象不要结论 |
| `model-audit` | 给定模型代码目录：分析模型/生成模型档案/核验描述与代码一致 |
| `result-eval` | 评估一次预测结果 / 算指标（默认 rmse_192）/ 月度或时段归因 / 深度分析 |
| `subset-influence` | N 个训练条目里哪些拖累留出目标（负迁移）/ chunk loss 震荡解释 |

## 引擎的运作方式

- **路由优先级（写死在 SKILL.md description，勿改丢——有 CI 守卫）**：已固化代理技能 > 本引擎。命中已固化场景直接短路用固化技能；其余诊断与评估目标一律由引擎的 9 个 playbook 覆盖。
...
- `/ts-diagnose` 并描述诊断目标（见上方 9 个 playbook 表）；引擎先跑 orient 定位阶段/匹配 playbook，
