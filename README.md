...
单一入口技能 —— 一个泛化时序诊断引擎，10 个可插拔 playbook，**两层结构**（生产者 playbook 产出持久化产物，分析 playbook 声明上游依赖消费之，orient 机器裁决先后）：

| 技能 | 用途 |
|------|------|
| [`ts-diagnose`](ts-diagnose/SKILL.md) | **泛化时序诊断引擎**：同一套已验证 workflow（orient 续跑、事实/解释隔离、结论三道门、多证据线、提问一等公民、生成闸+归因闸）+ 10 个可插拔 playbook（分两层：生产者/分析），覆盖时序/预测任务的全部诊断与评估目标；成熟运行可经 crystallize 固化成薄的已固化代理技能 |
...
**分析层（level-2）——声明上游产物依赖，各自只留目标核心菜谱：**
...
（分层机制 Phase 1 已落地 data-setup/model-audit/fact-scan 三生产者 + model-comparison 试点；其余分析 playbook 的批量改造见 `docs/superpowers/plans/2026-07-25-playbook-layering-phase2-TODO.md`。）

## 引擎的运作方式

- **路由优先级（写死在 SKILL.md description，勿改丢——有 CI 守卫）**：已固化代理技能 > 本引擎。命中已固化场景直接短路用固化技能；其余诊断与评估目标一律由引擎的 10 个 playbook 覆盖。分析 playbook 声明的必需上游产物（如 `setup`）缺失时 orient 打自动内联生产指令，可选上游（模型档案/图谱体检）缺失时三分支问用户——先后次序由引擎裁决，不靠使用者记。
...
- `/ts-diagnose` 并描述诊断目标（见上方 10 个 playbook 表）；引擎先跑 orient 定位阶段/匹配 playbook，
