...
## 适用问题
回答「模型的记忆有多长」。做法是按桶遮蔽历史：把一整桶输入置成基线值，
比逐点遮蔽便宜一个量级。Δ 相对**未遮蔽预测**度量，是纯依赖度量，与真值无关。
...
## JSON schema
见 frontmatter。四个桶从最近排向最远：最近 1 步、近四分位段、至主周期
（无周期时至半窗）、更远。桶边界由 lookback_steps 与 --period-steps 推出，
定义落 JSON 的 bucket_defs。
...
## 验证步
只读最近一步的适配器（L=8）：遮最近桶 Δ=1、其余桶 Δ=0，故
weights=[1,0,0,0]、short_term_share=1；--per-instance 下全窗 h*=1。
无 lookback 能力的线性适配器抛 ValueError。预算中途耗尽（如 max_calls
只够 base+前 2 桶）时逐项断言：delta_by_bucket 只保留已完成的桶，如实变短；
buckets_evaluated 对应变小；weights 与 short_term_share 置 null——桶不全
不做归一，避免误导；coverage.truncated=true。一个桶都没完成才抛
ValueError，已完成的桶不因超预算被整体丢弃。
