...
## 适用问题
「模型主要在用哪些输入」的量化答案。把每个特征当作 Shapley 博弈里的一名
玩家：mask=1 时该特征用实际序列，mask=0 时换成背景序列。KernelSHAP
（Shapley 值的核回归近似算法）据此分解每个 horizon 桶的输出。
...
## JSON schema
见 frontmatter。特征数 D≤8 时全枚举，结果精确、零随机；D>8 时改为采样，
seed 落盘。评估窗从 features∩predict 的完整窗里采样（--max-windows，seed 同源）。
...
## 验证步
线性适配器（系数 3/1/0）+ 8 窗双簇特征（0/1 各半，背景 k=2 → medoid 均值
0.5）。每窗贡献 = coef×(x−0.5)：overall 比值 fa/fb=3、fc≈0 精确回收；
by_bucket 每桶与 overall 同比值。另验预算截断：max_calls 压小时
truncated=true、windows_evaluated < 全量，不抛崩。
