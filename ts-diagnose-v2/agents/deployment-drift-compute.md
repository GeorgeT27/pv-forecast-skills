...
清单，跳过的图逐条注明缺材料原因。
用 `python3 scripts/orient.py --playbook deployment-drift` 领阶段与 prereq；
生成脚本前必过 `python3 scripts/gen_gate.py --script <path> --playbook deployment-drift
--stage <n>`（golden 在副本的 playbook 目录，脚本自动引用，卡片不内联菜谱正文）。
