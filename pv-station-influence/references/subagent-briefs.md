...
**何时**：orient 报预测侧上下文 [absent] 且用户同意先跑白马湖的 pv-result-analysis。
**为什么由主 agent 亲自编排**：subagent 不能再派 subagent，且主技能 Stage 3→4 的停顿
要问用户——所以嵌入运行的 Brief A/B 派发必须由主 agent 做。

```
1. mkdir <influence 工作目录>/result_analysis_baimahu/（独立目录——绝不写雅砻江线的
   任何已有分析目录）。在其中写 analysis_config.json：
     station      = "baimahu"
     metric_py    = <主线同一个 metric.py>
     true_label   = influence_config.test_label（同一份白马湖真值）
     predicted    = {M1..M4, ensemble: 白马湖【最终模型】预测 parquet}
                    ——influence_config 里没有这些路径，向用户要；
                      逐 chunk 预测属本技能 Stage 2 的 rmse_series，不进主技能流程。
     train_stations = {17 站: 逐站 parquet}（可选；给了本技能 Stage 4 漂移就白捡）
2. cd 该目录跑主技能 orient；随后照主技能 references/subagent-briefs.md 派发：
   Brief B（metric）×1 → 完成后 Brief A（figure+fact）×N 并行。
   参数覆盖：工作目录 = result_analysis_baimahu 绝对路径；Brief B 原文里
   "产物存 figures/yalongjiang/" 改为 figures/baimahu/（站名以 analysis_config.station 为准）。
...
```
你是一个计算子 agent，只负责【逐 checkpoint 重算白马湖 RMSE】，不做归因分析。
...
结合 station.md / event-log 的 Stage 4+ 工作），禁止把 loss 排名说成"拖累白马湖排名"。
```
...
1. 定位运行目录：
   - linked：cd <result_analysis_workdir>（其 analysis_config.json 已含 train_stations
     17 站路径）。若 figures/baimahu/drift/ 已有产物且新于配置 → 不重跑直接读。
   - 无 linked：在 influence 工作目录写一个**最小** analysis_config.json
     （station="baimahu"、true_label=influence_config.test_label、
     train_stations=<17 站逐站 parquet，向主 agent 要>）——该文件只服务 run_drift，
...
- [现象] 逐站 GHI PSI 排名（最像白马湖 → 最不像）| 数字：各站 PSI | 来源：drift 表
