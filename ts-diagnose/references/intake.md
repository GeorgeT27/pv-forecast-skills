...
4. **路径永远不许猜**：材料相关的证据自答（如工作目录已有明显的 predict parquet）
   只产生**候选**——必须 AskUserQuestion 向用户确认「发现 <路径>，这就是 <材料> 吗？」，
   确认后才可写 present；多候选属恒问五类⑤照问。且**免不了 schema 追问**——
   列名语义猜错污染全部下游（必问五类第 1 条）。
