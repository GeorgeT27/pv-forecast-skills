---
name: ts-diagnose
description: 泛化的时序/预测模型诊断与评估引擎。用户想知道：训练是否充分、batch 不足、loss 震荡收敛慢；结论或模型在扰动与分组切片下稳不稳；哪个输入变量对误差影响最大、反事实验证；为什么模型 A 比 B 好、模型对比归因；要消融实验验证模型组件；上线后是否退化、误差何时开始变大；只画标准分析图看现象不要结论；分析模型代码、生成模型档案；评估预测结果、算 RMSE 或指标表、月度时段归因；哪些训练条目拖累留出目标。按轮试改法提升模型指标、自动留好弃坏；命中后按本文件路由表转发。
---
...
2. **本引擎**——其余诊断与评估目标一律由下方 13 个 playbook 覆盖。
...
| 已有改进假设或可改的配置开关，想按轮试改法把指标提上去、留好弃坏、代码守预算 | `model-improve` |
...
材料时，升级到 `architecture-attribution` 做干预验证。干预验证后要把指标提上去，转 `model-improve`。
