...
2. **四个专用技能**次之——见上方 description 负面清单；
...
| 给模型代码目录生成「代码锚定」的模型参考档案，供归因机制消费（非光伏标准产线） | `model-audit` |
| 评估一次预测结果、算指标、把指标变化归因到时段/单元/输入（非光伏标准评估场景） | `result-eval` |
| 分块联合训练里哪个训练条目/数据子集拖累了留出单元的零样本预测、训练动力学解释（非光伏标准场景） | `subset-influence` |
