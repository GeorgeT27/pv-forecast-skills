...
## 适用问题
全图库唯一的朴素参照系：skill = 1 − RMSE_model/RMSE_baseline。它回答两个
问题：模型值不值得存在；深度模型是否退化成抄 persistence（persistence 即
「把发起时刻的真值原样当预测」的朴素基线）。
...
## JSON schema
见 frontmatter。真值全局序列按 target_ts=window_ts+step·freq 建。三条基线
分别是：persistence = 发起时刻真值 y(window_ts)；seasonal = y(target−period)，
即同相位上一周期的真值；climatology = 全局均值。基线值缺失的行跳过，
分母为 n_scored。

## 判读
- `skill_vs_persistence ≤ 0` → 候选：模型无增值。先查 copies_persistence：
  判据是预测离 persistence 基线的 RMSE 小于同行自身误差 RMSE 的一半。
  为真说明模型在「抄输入」，转 revision-stability 看翻新形态；
