...
- **训练/测试划分**：训练集 = 2024 全年，测试集 = 2025 全年，两者同构（同样的滚动窗口时序表）。训练集在本技能中不用于训练，而是**分布漂移诊断的基准**（见 `references/drift-and-nwp.md`）——回答"模型是不是没见过这种天"。
- **时间戳语义（关键，取点全靠它）**：
...
- 确认路径同时确认**这份数据属于哪个电站**；若两站混在一个文件，先弄清区分字段并拆分。

把路径与电站写进工作目录下的 `analysis_config.json`（供本次及续跑复用）：
...
`predicted` 按模型名记成字典；用户只给部分模型就只填有的。
...
3. 按 M1-M4 与 ensemble 分别运行，产出 5 个 Excel；两站分开跑、产物分开存。
...
图有两类读者：人 + 后续分析的模型。所以图要"可被视觉阅读"（关键数值标注在图上、固定配色、信息密度不过载），生成后必须 Read 回看形成闭环：读图 → 写结论（这张图说明什么、支持/否定哪个假设）→ 图读不清就重画 → 图与结论一起沉淀到 `figures/<电站>/<范围>/ANALYSIS.md`。只发图不给结论等于没分析。图输出目录 `figures/<电站>/<范围>/`（两站物理隔离），命名 `<图号>_<内容>_<范围>.png`。
