...
- **训练/测试都是 2025**：无跨年成分。训练集不参与指标计算，用途是**跨站漂移诊断的基准**（5 训练站 pooled + 逐站 vs 雅砻江，见 `references/drift-and-nwp.md`）——回答"模型是不是没见过这个站的这种天"。数据结构：训练与测试同构（同样的滚动窗口时序表）。
- **时间戳语义（关键，取点全靠它）**：
...
- 测试站固定是**雅砻江**；`train_set` 是 5 站联合训练集。确认训练集是**一个 pooled 文件**还是**逐站多个文件**（影响跨站逐站漂移，见 `train_stations`）。
...
```json
{
  ...
  "train_set": "<5 站联合训练集 parquet 绝对路径（pooled）>",
