...
跨站设定（2026-07-09）：train_set = 训练站集合 2025 pooled，true_label = 留出测试站。
漂移是**跨站**的——回答"留出站是不是落在训练站没覆盖的分布里、最像哪个训练站"。
...
用法（在有 analysis_config.json 的工作目录）：
    ...
    # 逐站对比：config 里给 "train_stations": {站名: 路径}，脚本对每个训练站单独 vs 留出站，
...
否则各自归一化会把"留出站整体更亮/更少波动"抹平。详见 references/drift-and-nwp.md。
"""
import argparse
import json
import os
...
def main():
    ...
    train = du.load_table(cfg["train_set"])    # 训练站集合 pooled 2025
    test = du.load_table(cfg["true_label"])    # 留出测试站 2025
    ...
    print("\n=== 五类天气占比：训练站(pooled) vs 留出站 ===")
    print(wd["share"])
    print("kt 漂移:", wd["kt"], "\nσΔ 漂移:", wd["sigma"])

    # 2b) 逐站对比（可选）：找留出站最像哪个训练站——按 GHI 的 PSI 排序
    if cfg.get("train_stations"):
        print("\n=== 逐站漂移：留出站 vs 各训练站（GHI PSI，小=最像）===")
        rows = []
